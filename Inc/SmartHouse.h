...
uint8_t     address = 0xFF;     // адрес устройства в системе (по умолчанию 0xFF)
...
int16_t     pwr;                // напряжение питания на входе устройства в мВ

#ifdef meas_num
typedef struct {                // таблица структур, содержащая данные измеряемых параметров
    uint8_t     name[9];        // строковое название измеряемого параметра (можно использовать кириллицу, 
                                // например "Гор.вода")
    uint8_t     unit[5];        // строковая запись единиц измерения (можно использовать кириллицу, например "л")
...
typedef struct {                // таблица структур, содержащая данные дискретно управляемых исполнительных устройств
    uint8_t     name[9];        // строковое название дискретно управляемого исполнительного устройства
...
typedef struct {                // таблица структур, содержащая данные плавно управляемых исполнительных устройств
    uint8_t     name[9];        // строковое название плавно управляемого исполнительного устройства
...
typedef struct {
    uint8_t     name[9];        // строковое название будильника (можно использовать кириллицу, например "Полив")
